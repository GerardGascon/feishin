...
export const useRadioListFilters = () => {
    const sortByFilter = useSortByFilter<RadioListSort>(
        RadioListSort.NAME,
        ItemListKey.RADIO,
    );
    const sortOrderFilter = useSortOrderFilter(null, ItemListKey.RADIO);
    ...
    const custom = useMemo(
        () => parseCustomFiltersParam(searchParams, FILTER_KEYS.RADIO.CUSTOM),
        [searchParams],
    );
